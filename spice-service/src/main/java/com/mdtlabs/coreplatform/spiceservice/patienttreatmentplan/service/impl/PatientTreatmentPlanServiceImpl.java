...
public class PatientTreatmentPlanServiceImpl implements PatientTreatmentPlanService {
    ...
    //  * {@inheritDoc}
    //  */
    // public PatientTreatmentPlan getPatientTreatmentPlan(Long patientTrackId, boolean isDeleted) {
    //     List<PatientTreatmentPlan> treatmentPlan = patientTreatmentPlanRepository.findByPatientTrackIdAndIsDeletedOrderBymodifiedAtDesc(
    //             patientTrackId,
    //             isDeleted);
    ...
    public PatientTreatmentPlan getPatientTreatmentPlan(Long patientTrackId) {
        ...
        List<PatientTreatmentPlan> treatmentPlan = patientTreatmentPlanRepository.findByPatientTrackIdAndIsDeletedOrderByUpdatedAtDesc(
                patientTrackId,
                false);
        return treatmentPlan.isEmpty() ? null : treatmentPlan.get(0);
    }
    ...
    public PatientTreatmentPlan getPatientTreatmentPlanDetails(Long id, Long tenantId) {
        PatientTreatmentPlan treatmentPlan = patientTreatmentPlanRepository
                .getTreatementPlanDetails(id, tenantId);
        return treatmentPlan;
    }

    // public void getNextFollowupDate(long patientTrackId, String frquencyName) {
    //     PatientTreatmentPlan patientTreatmentPlan = patientTreatmentPlanRepository.getTreatementPlanDetails(patientTrackId, null);
    //     return get
    // }
    ...
    public PatientTreatmentPlan getPatientTreatmentPlanDetails(Long id, String cvdRiskLevel, Long tenantId) {
        PatientTreatmentPlan treatmentPlan = patientTreatmentPlanRepository
                .findByPatientTrackIdAndTenantId(id, tenantId);
        return treatmentPlan;
    }
    ...
    public List<Map<String, String>> createProvisionalTreatmentPlan(PatientTracker patientTracker, String cvdRiskLevel,
        ...
        System.out
                .println("*************************************createProvisionalTreatmentPlan,,treatmentPlan:"
                        + treatmentPlan);
        if (Objects.isNull(treatmentPlan)) {
...
//                    treatmentPlan.setBgCheckFrequency(frequency.getFrequencyName());
                        treatmentPlanDurations.put(Constants.BG_CHECK_FREQUENCY,
                                Constants.BG_PROVISIONAL_FREQUENCY_NAME);
                        break;
...
//        patientTracker.setNextMedicalReviewDate(followupDates.get(Constants.FREQUENCY_MEDICAL_REVIEW));
    ...
    public boolean updateTreatmentPlanData(PatientTreatmentPlan patientTreatmentPlan) {
        ...
        if(Objects.isNull(patientTreatmentPlan.getId())) {
            ...
            PatientTreatmentPlan existPatientTreatmentPlan = patientTreatmentPlanRepository.findByIdAndIsDeleted(patientTreatmentPlan.getId(), Constants.BOOLEAN_FALSE);
