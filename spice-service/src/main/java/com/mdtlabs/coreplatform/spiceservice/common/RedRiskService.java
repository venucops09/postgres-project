...
public class RedRiskService {
		...
		if (!Objects.isNull(redRiskDTO.getDiabetesPatientType())
			...
			if (!Objects.isNull(patientDiagnosis)) {
				patientDiagnosis = patientDiagnosisRepository
						.findByPatientTrackIdAndIsActiveAndIsDeleted(patientTracker.getId(), true, false);
			}
			System.out.println("+===================================patientDiagnosis: " + patientDiagnosis);
			List<PatientComorbidity> patientComorbidities = patientComorbidityRepository
					.getBytrackerId(patientTracker.getId(), Constants.BOOLEAN_TRUE, Constants.BOOLEAN_FALSE);
	...
	public static RedRiskNotification createRedRiskNotification(RedRiskNotification redRiskNotification) {
		// Create red risk notification. patient tracker ID:
		// ${notificationData.patient_track_id}
	...
	public static void updateRedRiskNoticationStatus(Long patientTrackId) {
		redRiskNotificationRepository.updateRedRiskStatus(Constants.MEDICAL_REVIEW_COMPLETED, patientTrackId,
				Constants.NEW);
	}

	/*
	 * onst updateRedRiskNoticationStatus = async (app, params, patientTrackerId) =>
	 * { try { log.info(`Update the red risk notification status to completed.
	 * patient tracker ID: ${patientTrackerId}`); const redriskUpdateData = {
	 * status: AppConstants.REDRISK_NOTIFICATION_STATUS.MEDICAL_REVIEW_COMPLETED };
	 * const copyParams = core_util.cloneParams(params); copyParams.query = {
	 * patient_track_id: patientTrackerId, is_deleted: false, status:
	 * AppConstants.REDRISK_NOTIFICATION_STATUS.NEW, is_multi_update: true,
	 * tenant_id: params.query.tenant_id }; copyParams.route.modelName =
	 * AppConstants.MODEL.RED_RISK_NOTIFICATION; await
	 * app.service('forms/:modelName').patch(null, redriskUpdateData, copyParams); }
	 * catch (error) { log.error(`Error while update the red risk notfication status
	 * - ${error}`); throw error; }
	 */
}
