...
public interface BpLogRepository extends JpaRepository<BpLog, Long> {
    ...
    static final String GET_BP_LOG_BY_PATIENT_TRACK_ID =
            "select bplog from BpLog as bplog where " +
                    "bplog.patientTrackId = :patientTrackId and isLatest=true and " +
                    "bplog.createdAt > :yesterday";

    public static final String UPDATE_BP_LOG_LATEST_STATUS =
    "update BpLog as bplog set bplog.isLatest=:isLatest where bplog.patientTrackId=:patientTrackId";

    static final String GET_BP_LOGS =
    "Select * from bplog where patient_track_id=:patientTrackId and " +
     ...
     * @return BpLog entity
     */
    public BpLog findBypatientTrackIdAndIsDeletedAndIsLatest(long patientTrackId,
    ...
    BpLog findByPatientTrackIdAndIsCreatedToday(@Param("patientTrackId") long patientTrackId, @Param("yesterday") Date date);
