...
public class RiskAlgorithm {
	...
	public static String calculateRiskLevelFromGlucoseData(Float glucoseValue, String glucoseType,
		...
		if (existingRiskLevel != Constants.HIGH
				&& ((glucoseType == Constants.RBS && (glucoseValue >= 13 || glucoseValue < 4))
						|| (glucoseType == Constants.FBS && (glucoseValue > 11 || glucoseValue < 4))
						|| (glucoseType == Constants.RBS && (glucoseValue > 11 || glucoseValue < 4)))) {
			existingRiskLevel = Constants.HIGH;
		}
		if ((glucoseType == Constants.RBS && glucoseValue > 10 && glucoseValue < 12.9)
				|| (glucoseType == Constants.FBS && glucoseValue > 7.8 && glucoseValue < 11)
				|| (glucoseType == Constants.RBS && glucoseValue > 7.8 && glucoseValue < 11)) {
	...
	public static String getRiskLevelForNewPatient(PatientTracker patientTracker, RedRiskDTO redRiskDTO) {
		String riskLevel = null;
		if (patientTracker.getIsPregnant()) {
			riskLevel = Constants.HIGH;
		} else if (Constants.ONE <= redRiskDTO.getComorbiditiesCount() || redRiskDTO.getDiabetesDiagControlledType()
	...
	public static int getRiskFactorsCount(PatientTracker patientTracker) {
		...
		if (patientTracker.getGlucoseType() == Constants.FBS && patientTracker.getGlucoseValue() >= 5.6
	...
	private static String calculateRiskFromBPAndRiskFactors(PatientTracker patientTracker, int riskFactorsCount,
		...
		if (patientTracker.getAvgSystolic() >= 180 || patientTracker.getAvgDiastolic() >= 110) {
			...
			if (riskFactorsCount > 0 || redRiskDTO.getDiabetesDiagControlledType() == Constants.DIABETES_WELL_CONTROLLED
