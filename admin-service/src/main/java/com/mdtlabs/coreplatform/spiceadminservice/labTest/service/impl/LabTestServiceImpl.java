...
public class LabTestServiceImpl implements LabTestService {
	...
	public List<Map> searchLabTests(RequestDTO requestDTO) {
		...
		List<LabTest> labTests = labTestRepository.searchLabTests(formattedSearchTerm, requestDTO.getCountryId(),
				requestDTO.getIsActive());
		return labTests.stream().map(
				labTest -> Map.of("id", labTest.getId(), "name", labTest.getName(), "country", labTest.getCountryId()))
...
// =======
	...
	// List<LabTestResult> oldInactiveResults =
	// existingLabTest.getLabTestResults().stream()
	// .filter(labTestResult ->
	// !labTestResult.getIsActive()).collect(Collectors.toList());
	...
	private void validateLabTestResultsAndUpdate(LabTest labTest, LabTest existingLabTest) {
		...
		List<String> names = newLabTestResults.stream().map(name -> name.getName()).collect(Collectors.toList());
